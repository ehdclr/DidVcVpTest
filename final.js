...
async function run()
{
    ...
    let stewardDidInfo = {
        'seed': '0000000000000000000000000000jong'
    };
    ...
    // await indy.proverFetchCredentialsForProofReq(searchForidCertificateProofRequest, 'attr5_referent', 100)
...
async function proverGetEntitiesFromLedger(poolHandle, did, identifiers, actor,revState,timestamp) {
    ...
    for(let referent of Object.keys(identifiers)) {
        ...
        //     console.log('조건문 될까? :', item.rev_reg_seq_no);
        //     console.log(`\"${actor}\" -> rev 객체 `);
        //     let [receivedrevRegId, receivedrevRegDef] = await getRevocReg(poolHandle, did, item['rev_reg_id']);
        //     revStates[receivedrevRegId] = receivedrevRegDef;

        //     console.log('한번더~ :', revStates);
